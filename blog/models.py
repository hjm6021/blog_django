...
class Category(models.Model):
    name = models.CharField(max_length=50, help_text="블로그 글의 분류를 입력하세요.(ex: 일상, 유머, 정보)")

    def __str__(self):
        return self.name

# 블로그 글(제목, 작성일, 대표, 이미지, 내용, 카테고리)
class Post(models.Model):
    title = models.CharField(max_length=200)
    title_image = models.ImageField(blank=True)
    content = models.TextField()
    createDate = models.TimeField(auto_now_add=True)
    updateDate = models.TimeField(auto_now_add=True)
    # 하나의 글은 여러가지의 분류에 해당될 수 있다. manytomanyfield
